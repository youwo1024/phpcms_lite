...
class index extends foreground {
	...
	public function send_newmail() {
		...
		//判断会员表
		$r = $this->db->get_one(array('email'=>$newemail));
	...
	public function account_manage() {
		...
		foreach($model_info as $k=>$v) {
			...
			if($v['formtype'] == 'image') {
				$member_modelinfo[$v['name']] = "<a href='$member_modelinfo_arr[$k]' target='_blank'><img src='$member_modelinfo_arr[$k]' height='40' widht='40' onerror=\"this.src='".IMG_PATH."nophoto.gif'\"></a>";
				...
				if(is_array($tmp)) {
					foreach ($tmp as $tv) {
						$member_modelinfo[$v['name']] .= " <a href='$tv[url]' target='_blank'><img src='$tv[url]' height='40' widht='40' onerror=\"this.src='".IMG_PATH."nophoto.gif'\"></a>";
					}
					unset($tmp);
				}
	...
	public function account_manage_avatar() {
		...
		if ($postStr) {
			...
			$this->db->update(array('avatar' => 1), array('userid' => $userid));
			exit('1');
		}

		$upurl = base64_encode('index.php?m=member&c=index&a=account_manage_avatar');
		//获取头像数组
	...
	public function friend() {
		...
		if(isset($_GET['friendid'])) {
			...
			foreach($friendids as $k=>$v) {
				...
				$friendlist[$k]['avatar'] = get_memberavatar($v['userid']);
